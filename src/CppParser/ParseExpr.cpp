...
AST::Expr* Parser::WalkExpression(const clang::Expr* Expr)
{
    ...
    case clang::Stmt::ConstantExprClass:
    {
        ...
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
        _S->isInstantiationDependent = S->isInstantiationDependent();
        _S->containsUnexpandedParameterPack = S->containsUnexpandedParameterPack();
        _S->containsErrors = S->containsErrors();
        _S->isReadIfDiscardedInCPlusPlus11 = S->isReadIfDiscardedInCPlusPlus11();
        _S->isLValue = S->isLValue();
        _S->isPRValue = S->isPRValue();
        ...
        _S->resultStorageKind = (AST::ConstantExpr::ResultStorageKind)S->getResultStorageKind();
        _S->isImmediateInvocation = S->isImmediateInvocation();
        _S->hasAPValueResult = S->hasAPValueResult();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::DeclRefExprClass:
    {
        ...
        _S->isNonOdrUse = (AST::NonOdrUseReason)S->isNonOdrUse();
        _S->refersToEnclosingVariableOrCapture = S->refersToEnclosingVariableOrCapture();
        _S->isImmediateEscalating = S->isImmediateEscalating();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::CharacterLiteralClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->kind = (AST::CharacterLiteral::CharacterKind)S->getKind();
        _S->value = S->getValue();
        _Expr = _S;
        break;
    }
    case clang::Stmt::FloatingLiteralClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->rawSemantics = (AST::FloatSemantics)S->getRawSemantics();
        _S->exact = S->isExact();
    ...
    case clang::Stmt::StringLiteralClass:
    {
        ...
        _S->kind = (AST::StringLiteral::StringKind)S->getKind();
        _S->isOrdinary = S->isOrdinary();
        _S->isWide = S->isWide();
        _S->isUTF8 = S->isUTF8();
        _S->isUTF16 = S->isUTF16();
        _S->isUTF32 = S->isUTF32();
        _S->isUnevaluated = S->isUnevaluated();
    ...
    case clang::Stmt::PredefinedExprClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->identKind = (AST::PredefinedExpr::IdentKind)S->getIdentKind();
        _S->isTransparent = S->isTransparent();
        _S->identKindName = S->getIdentKindName();
        _Expr = _S;
        break;
    }
    case clang::Stmt::SYCLUniqueStableNameExprClass:
    {
        auto S = const_cast<clang::SYCLUniqueStableNameExpr*>(llvm::cast<clang::SYCLUniqueStableNameExpr>(Expr));
        auto _S = new AST::SYCLUniqueStableNameExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    case clang::Stmt::UnaryOperatorClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->opcode = (AST::UnaryOperatorKind)S->getOpcode();
        _S->subExpr = static_cast<AST::Expr*>(WalkExpression(S->getSubExpr()));
    ...
    case clang::Stmt::UnaryExprOrTypeTraitExprClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->kind = (AST::UnaryExprOrTypeTrait)S->getKind();
    ...
    {
        auto S = const_cast<clang::MatrixSubscriptExpr*>(llvm::cast<clang::MatrixSubscriptExpr>(Expr));
        auto _S = new AST::MatrixSubscriptExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    case clang::Stmt::CallExprClass:
    {
        ...
        _S->aDLCallKind = (AST::CallExpr::ADLCallKind)S->getADLCallKind();
        _S->usesADL = S->usesADL();
        _S->hasStoredFPFeatures = S->hasStoredFPFeatures();
    ...
    case clang::Stmt::ImplicitCastExprClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->castKind = (AST::CastKind)S->getCastKind();
    ...
    case clang::Stmt::BinaryOperatorClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->opcode = (AST::BinaryOperatorKind)S->getOpcode();
        _S->lHS = static_cast<AST::Expr*>(WalkExpression(S->getLHS()));
        _S->rHS = static_cast<AST::Expr*>(WalkExpression(S->getRHS()));
    ...
    case clang::Stmt::StmtExprClass:
    {
        ...
        _S->templateDepth = S->getTemplateDepth();
        _Expr = _S;
        break;
    }
    ...
    {
        auto S = const_cast<clang::SourceLocExpr*>(llvm::cast<clang::SourceLocExpr>(Expr));
        auto _S = new AST::SourceLocExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->builtinStr = S->getBuiltinStr();
        _S->identKind = (AST::SourceLocExpr::IdentKind)S->getIdentKind();
        _S->isIntType = S->isIntType();
        _Expr = _S;
        break;
    }
    case clang::Stmt::InitListExprClass:
    {
        ...
        _S->hasDesignatedInit = S->hasDesignatedInit();
    ...
    case clang::Stmt::DesignatedInitExprClass:
    {
        ...
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->size = S->size();
        _S->isDirectInit = S->isDirectInit();
    ...
    case clang::Stmt::GenericSelectionExprClass:
    {
        ...
        _S->isExprPredicate = S->isExprPredicate();
        _S->isTypePredicate = S->isTypePredicate();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::AtomicExprClass:
    {
        ...
        _S->op = (AST::AtomicExpr::AtomicOp)S->getOp();
        _S->numSubExprs = S->getNumSubExprs();
    ...
    {
        auto S = const_cast<clang::RecoveryExpr*>(llvm::cast<clang::RecoveryExpr>(Expr));
        auto _S = new AST::RecoveryExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    case clang::Stmt::CXXOperatorCallExprClass:
    {
        ...
        _S->_operator = (AST::OverloadedOperatorKind)S->getOperator();
        _S->isAssignmentOp = S->isAssignmentOp();
        _S->isComparisonOp = S->isComparisonOp();
    ...
    case clang::Stmt::CXXMemberCallExprClass:
    {
        ...
        _S->objectType = GetQualifiedType(S->getObjectType());
    ...
    {
        auto S = const_cast<clang::CXXRewrittenBinaryOperator*>(llvm::cast<clang::CXXRewrittenBinaryOperator>(Expr));
        auto _S = new AST::CXXRewrittenBinaryOperator();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    {
        auto S = const_cast<clang::CXXAddrspaceCastExpr*>(llvm::cast<clang::CXXAddrspaceCastExpr>(Expr));
        auto _S = new AST::CXXAddrspaceCastExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    case clang::Stmt::UserDefinedLiteralClass:
    {
        ...
        _S->literalOperatorKind = (AST::UserDefinedLiteral::LiteralOperatorKind)S->getLiteralOperatorKind();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::CXXUuidofExprClass:
    {
        ...
        _S->isTypeOperand = S->isTypeOperand();
        _S->exprOperand = static_cast<AST::Expr*>(WalkExpression(S->getExprOperand()));
        _S->guidDecl = S->getGuidDecl()->getNameAsString();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::CXXDefaultArgExprClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->hasRewrittenInit = S->hasRewrittenInit();
        _Expr = _S;
        break;
    }
    ...
    case clang::Stmt::LambdaExprClass:
    {
        ...
        _S->dependentCallOperator = static_cast<AST::FunctionTemplate*>(WalkDeclaration(S->getDependentCallOperator()));
        _S->trailingRequiresClause = static_cast<AST::Expr*>(WalkExpression(S->getTrailingRequiresClause()));
    ...
    case clang::Stmt::CXXNewExprClass:
    {
        ...
        _S->initializationStyle = (AST::CXXNewExpr::InitializationStyle)S->getInitializationStyle();
    ...
    case clang::Stmt::CXXFoldExprClass:
    {
        ...
        _S->refersToMatrixElement = S->refersToMatrixElement();
        _S->hasPlaceholderType = S->hasPlaceholderType();
        _S->callee = static_cast<AST::UnresolvedLookupExpr*>(WalkExpression(S->getCallee()));
        _S->lHS = static_cast<AST::Expr*>(WalkExpression(S->getLHS()));
        _S->rHS = static_cast<AST::Expr*>(WalkExpression(S->getRHS()));
    ...
    {
        auto S = const_cast<clang::CXXParenListInitExpr*>(llvm::cast<clang::CXXParenListInitExpr>(Expr));
        auto _S = new AST::CXXParenListInitExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
    ...
    {
        auto S = const_cast<clang::BuiltinBitCastExpr*>(llvm::cast<clang::BuiltinBitCastExpr>(Expr));
        auto _S = new AST::BuiltinBitCastExpr();
        _S->dependence = (AST::ExprDependence)S->getDependence();
        _S->type = GetQualifiedType(S->getType());
        _S->isValueDependent = S->isValueDependent();
        _S->isTypeDependent = S->isTypeDependent();
